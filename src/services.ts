...
export const getEvents = async (fromBlock: number, toBlock: string | undefined, fileName: string) => {
    ...
    console.log('firstLoopEvents', firstLoopEvents.length)
...
export const uniqueToAndFrom = async() => {
  ...
  console.log('collection', db.datas)
  // db.collection('data').rename('datas').then((res: any) => console.log('res', res)).catch((err: any) => console.log('err', err))

  const Model = mongoose.model("datas", schema)

  Model.findOne().then((res: any) => console.log('res', res))
  // Model.find({Block_Number:'11185311'}).then((res: any) => console.log('res', res)).catch((err: any) => console.log('err', err))

  // Model.find().distinct('Tx_Hash').limit(20).exec(function(err: any, data: any) {
    // console.log('data', data)
    // `posts` will be of length 20
// });

  // const res = await db.db["eth_validator"].aggregate([ {$group: { Tx_Hash: "$uuid" } } ],{allowDiskUse:true}).itcount()
  // console.log('res', res)

  // db.db.datas.aggregate( [ { $group : { _id : "$price" } } ] ).then((res: any) => console.log('res', res))
  const res =  await Model.aggregate([
    { 
      "$group": { 
        "_id": "$To",
      //   "BlockNumber": {
      //     $addToSet: "$Block_Number",        
      //  },
      //  "From": {
      //   $addToSet: "$From",
      //  },
  ...
  // .then((val: any) => console.log('val', val))
  console.log('res', res)
...
export const rapidNetwork = async () => {
  ...
  // var config = {
  //   method: 'get',
  //   url: 'https://api.rated.network/v0/eth/validators/0x8efba2238a00d678306c6258105b058e3c8b0c1f36e821de42da7319c4221b77aa74135dab1860235e19d6515575c381',
  //   headers: { 
  //     'accept': 'application/json', 
  //     'X-Rated-Network': 'mainnet', 
  //     'Authorization': 'Bearer eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJzY29wZXMiOltdLCJpZCI6ImE0ODFkY2M3M2QxNTQ2Y2RiYjI1ZGE1YTEzZjJhNWU5Iiwic3ViIjoiZDM0ZTYxMmFhYjk5NDc3YTk0MWY5MGQ3ZWI0M2Q0MzYiLCJleHAiOjE3MDYwMDg5OTN9.hZZbF86R3J52v8HYQ6NfpsUrBFuzSkCON29vORIOnayH2Ly1a-ZXK8hVPlGwrXqm2gHAhxGKb5pmwbAegNPzzQ'
